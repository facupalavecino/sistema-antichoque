...
#define SSID        "SistemaAntichoque" //nombre de la RED
#define PASSWORD    "12345678" //PASSWORD de la red (no necesariemnte se requeire )
#define PORT        666 //puerto del servidor
#define ENCRYPTION  0   //modo de enciptacion(ninguno)
#define CHANNEL     3   //canal 3
#define TAM_BUFFER  128   //canal 3
#define SERVER_TIMEOUT  800   //canal 3
...
void ESP8266::Configuration()
 {
        
        setOprToSoftAP(); 
        setSoftAPParam( SSID , PASSWORD,CHANNEL,ENCRYPTION);
        enableMUX();
        startTCPServer(PORT);
        setTCPServerTimeout(SERVER_TIMEOUT);
    }

 void ESP8266::Comprobation()
{
    //comprueba dos veces que no responda con un 250 ms de delay entre consulta
      /* if (kick()==0){ 
            delay(250);
            if (kick()== 0){
                restart();
 ...
 uint32_t ESP8266::Receive_data(uint8_t *buffer){

      uint8_t mux_id;  
      uint32_t len = recv(&mux_id, buffer, 128 , 500); //128=tamaño de buffer, 500=Timeout de espera

      return len;
 }

 void ESP8266::Send_data(uint8_t *buffer, uint32_t len){
    uint8_t mux_id; 
